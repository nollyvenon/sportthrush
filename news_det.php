...
$news_id = $_GET['sid'];
$query = "SELECT * FROM news WHERE news_id='$news_id' order by news_id DESC ";
$result = $db_handle->runQuery($query);
$news = $db_handle->fetchAssoc($result);
extract($news[0]);
?><!DOCTYPE html>
						...
						<div class="single-post nobottommargin">
							...
							<div class="entry clearfix">
								...
								<div class="entry-content notopmargin">
									...
									<div class="entry-image alignleft">
										<?php
										$post_images = $client_operation->get_gallery($news_id);
										if(isset($post_images) && !empty($post_images)) { foreach ($post_images as $row22) {
													$image_url = $row22['image_url'];?>
										<a href="images/news/<?=$image_url;?>"><img src="images/news/<?=$image_url;?>" alt="<?=$title;?>"></a>
												<?php } }?>
									</div><!-- .entry-image end -->

									<br><?php echo $news_content;?>
							...
							<div class="post-navigation clearfix">
								...
								<div class="col_half nobottommargin">
									<a href="?newsdet?sid=<?=$news_id-1;?>">&lArr; Prev</a>
								</div>

								<div class="col_half col_last tright nobottommargin">
									<a href="?newsdet?sid=<?=$news_id+1;?>">Next &rArr;</a>
								</div>
							...
							<div class="related-posts clearfix">
								...
								<div class="col_half nobottommargin">
									<?php
									$query = "SELECT * FROM news order by RAND() DESC LIMIT 1,3";
									$result = $db_handle->runQuery($query);
									$news_random = $db_handle->fetchAssoc($result);
							if(isset($news_random) && !empty($news_random)) { foreach ($news_random as $row) {
							?>
								...
								<div class="col_half nobottommargin col_last">
									<?php
									$query = "SELECT * FROM news order by RAND() DESC LIMIT 4,5";
									$result = $db_handle->runQuery($query);
									$news_random = $db_handle->fetchAssoc($result);
							if(isset($news_random) && !empty($news_random)) { foreach ($news_random as $row) {
							?>
