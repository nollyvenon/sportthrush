...
class LifeHelpSystem {
    ...
    public function send_email($subject, $message, $sendto_email, $sendto_name, $from_name = '') {
        ...
        $mail = new PHPMailer;
        $mail->IsSMTP();
        $mail->Host = "life@lifehelp.club";  // specify main and backup server
        $mail->SMTPAuth = true;     // turn on SMTP authentication
        $mail->Username = "life@lifehelp.club";  // SMTP username
        $mail->Password = "wedu123"; // SMTP password

        //From email address and name
        $mail->From = "support@lifehelp.club";
        
        if(isset($from_name) && !empty($from_name)) {
            $mail->FromName = $from_name;
            
            //Address to which recipient will reply
            $mail->addReplyTo("support@lifehelp.club", $from_name);
        } else {
            $mail->FromName = "LifeHelp Club";
            
            //Address to which recipient will reply
            $mail->addReplyTo("support@lifehelp.club", "LifeHelp Club");
        }
